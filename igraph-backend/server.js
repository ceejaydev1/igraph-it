...
// X-Forwarded-For header, so every request looks like it comes from the same IP
// and all the rate limiters below end up bucketing every visitor together.
// `1` trusts exactly one hop (Render's own proxy), which is the correct value
// for this setup — not `true`, which would trust an attacker-supplied
// X-Forwarded-For if this app were ever reachable directly.
...
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    if (durationMs > 1000) {
      console.warn(
        `🐢 SLOW REQUEST: ${req.method} ${req.originalUrl} — ${durationMs}ms — status ${res.statusCode}`
      );
    }
  });
...
const allowedOrigins = [
  // ✅ Production frontend
  'https://igraphit.com',
  'https://www.igraphit.com',

  // Existing production/deployment origins
  ...
  // Local network / Expo development
...
const corsOptions = {
  origin: function (origin, callback) {
    ...
    // (for example native/mobile requests)
    if (!origin) {
    ...
    const isLocal =
      /^https?:\/\/(localhost|127\.0\.0\.1|192\.168\.|10\.0\.0\.)/.test(origin);

    // Allow Netlify preview/deployment origins
